...
public class MockServerTests {
	...
	public void getAllContacts() throws Exception {
		mockMvc.perform(get(addressBookUrl + "contacts").accept(contentType)).andExpect(status().isOk())
	...
	public void findContactById() throws Exception {
		...
		mockMvc.perform(get(addressBookUrl + "contactdetails/{id}", randomContact.getId()).accept(contentType))
				...
				.andExpect(jsonPath("$.aFirstName", is(randomContact.getFirstName())))
				.andExpect(jsonPath("$.aLastName", is(randomContact.getLastName())))
	...
	public void findContactsByNotExistingFirstName() throws Exception {
		...
		mockMvc.perform(get(addressBookUrl + "/contactdetails?aFirstName={aFirstName}", impropableNameToApper)
	...
	public void createNotExistingContact() throws Exception {
		ContactDetails demoContact = createDemoContact();
		mockMvc.perform(post(addressBookUrl + "contactdetails").contentType(contentType)
				...
				.andExpect(content().contentType(contentType)).andDo(result -> {
					System.out.println("Response: " + result.getResponse().getContentAsString());
				}).andExpect(jsonPath("$.aFirstName", is(demoContact.getFirstName())))
				.andExpect(jsonPath("$.aLastName", is(demoContact.getLastName())))
	...
	public void createContactTwice() throws Exception {
		...
		mockMvc.perform(get(addressBookUrl + "contactdetails/{id}", id1).accept(contentType)).andExpect(status().isOk())
				.andExpect(content().contentType(contentType)).andDo(result -> {
					System.out.println("Response: " + result.getResponse().getContentAsString());
	...
	private ContactDetails createDemoContact() {
		String firstName = "demoName";
		String lastName = "demoSurname";
		String phone = "+61-426-123-999";
		return new ContactDetails(firstName, lastName, phone);
	}

	private IndexedContactDetailsInfo getRandomContact() {
		Random randomIndexGenerator = new Random();
		int randomContactIndex = randomIndexGenerator.nextInt(contactDetailsInfo.size());
